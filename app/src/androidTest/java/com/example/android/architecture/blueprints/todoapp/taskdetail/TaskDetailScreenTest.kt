...
class TaskDetailScreenTest {
    ...
    @Test
    fun completedTaskDetails_isDisplayedAsChecked() = runTest {
        // GIVEN: Uma tarefa completada no repositório
        val title = "Task Completa"
        val desc = "Descricao Task Completa"
        val taskId = repository.createTask(title, desc)
        repository.completeTask(taskId)

        // WHEN: Abrimos a tela de detalhes dessa task
        setContent(taskId)

        // THEN: Verificamos se o título e a descrição aparecem
        composeTestRule
        ...
        // Verificar se o Checkbox está marcado (No Compose usamos o ToggleableState)
        // Buscamos pelo componente que pode ser marcado/desmarcado
        composeTestRule
            .onNode(isToggleable())
            .assertIsOn()
    }
